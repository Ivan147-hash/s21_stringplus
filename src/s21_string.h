...
typedef struct {  // часть отформатированой строки, от % до спецификатора
  char spec;  //-1 дефолтное значение -2 звездочка -3 несущетвует инстанса
  char *flags;
  int width;
  int accuracy;
  char lenght;
...
typedef struct {  // тема выше + обычная строка, чтоб удобно было возвращать из
                  // парсящих функций
  char *str;
