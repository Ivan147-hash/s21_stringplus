...
char* cut_str(const char* format, int start, int end) {
  ...
  char* cpy = malloc(lenght + 1 * sizeof(char));
...
int s21_sprintf(char* str, const char* format, ...) {
  ...
  for (i = 0; i < n; i++) {
    ...
    char len = parsed[i].format.lenght;
  ...
  if (width != -1 && spaces > 0) {
    *str = realloc(*str, len + 1);
    if (is_in_arr(flags, '-')) {
      ...
      for (int i = plus; (s21_size_t)i < s21_strlen(*str); i++) {
        cp[i] = (*str)[i];
      }
      int sn = s21_strlen(*str);
...
char* int_to_str(long int inp) {
  ...
  int minus = 0;
  if (inp < 0) minus = 1;
  char* rez = malloc((i + 2 + minus) * sizeof(char));
  if (inp < 0) rez[0] = '-';
...
char* float_to_str(double inp, int size) {
  ...
  del /= 10;
  int mul = 10;
  char* res = calloc((i + size + 2), sizeof(char));
  if (inp < 0) {
    res[0] = 45;
    f = 1;
    tm *= -1;
